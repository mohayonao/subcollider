...
export function clip(x, lo, hi) {
  lo = utils.defaults(lo, -1);
  hi = utils.defaults(hi, +1);
...
export function clip2(x, range) {
  let hi = Math.abs(utils.defaults(range, 1));
  let lo = -hi;
...
export function cpsmidi(cps, baseFreq) {
  baseFreq = utils.defaults(baseFreq, config.baseFreq);
...
// # sc.gauss(x, standardDeviation, rand=sc.config.randomAPI)
// a gaussian distributed random number
export function gauss(x, standardDeviation, rand) {
  rand = utils.defaults(rand, config.randomAPI);

  return (Math.sqrt(-2 * Math.log(rand())) * Math.sin(rand() * Math.PI * 2)) * standardDeviation + x;
}

// sc.gaussCurve(x, a=1, b=0, c=1)
// map the receiver onto a gauss function
export function gaussCurve(x, a, b, c) {
  a = utils.defaults(a, 1);
  b = utils.defaults(b, 0);
  c = utils.defaults(c, 1);

  return a * Math.exp(squared(x - b) / (-2.0 * squared(c)));
}
...
// # sc.sum3rand(x, rand=sc.config.randomAPI)
export function sum3rand(x, rand) {
  rand = utils.defaults(rand, config.randomAPI);

  return (rand() + rand() + rand() - 1.5) * (2 / 3) * x;
}
